...
| M3 | 个股排序模型 | 训练 `rank_strength_score` | 打败随机/动量基准 |
...
实现 `01_train_stock_rank_model.py`，训练主模型 `rank_strength_score`。
...
```text
Top3 扣费后平均收益 > 随机 Top3。
Top3 扣费后平均收益 >= 近5日动量 Top3。
Top3 命中 Top30 平均数量 > 随机 Top3。
Top3 命中 Top10 有改善。
收益不是由少数极端日期贡献。
```
