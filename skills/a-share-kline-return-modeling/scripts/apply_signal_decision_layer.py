...
def decide_action(day: pd.DataFrame, args: argparse.Namespace) -> tuple[str, int, str]:
    ...
    if (
        top["aggregate_market_risk_level"].iloc[0] == "高风险"
...
def main() -> None:
    ...
    if final.empty:
        ...
        avg_return = final["future_5_return"].mean() if "future_5_return" in final.columns else np.nan
        up_rate = (final["future_5_return"] > 0).mean() if "future_5_return" in final.columns else np.nan
        print(f"最终信号: rows={len(final)}, dates={final['trade_date'].nunique()}, up_rate={up_rate:.4f}, avg_return={avg_return:.4f}")
