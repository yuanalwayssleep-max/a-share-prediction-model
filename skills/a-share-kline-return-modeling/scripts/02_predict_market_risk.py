...
def market_regime(row: pd.Series) -> str:
    if row.get("is_high_position_reversal_risk", 0) == 1:
        return "高位回落"
    if row.get("is_exhausted_rebound_risk", 0) == 1:
        return "超跌反弹"
...
def predict_one_anchor(
    ...
    out["is_high_position_reversal_risk"] = (
        (out["pct_chg"] >= 0.6)
        & (out["index_ret_5"] >= 0)
        & (out["index_range_pos_20"] >= 0.55)
        & (out["predicted_down_prob"] < 0.55)
    ).astype(int)
    out["is_exhausted_rebound_risk"] = (
        (out["pct_chg"] >= 0.5)
        & (out["index_ret_5"] < 0)
        & (out["index_range_pos_20"] < 0.60)
        & (out["predicted_up_prob"] >= 0.85)
    ).astype(int)
    ...
    out.loc[out["is_high_position_reversal_risk"].eq(1), "market_risk_level"] = "高风险"
    out.loc[out["is_exhausted_rebound_risk"].eq(1), "market_risk_level"] = "高风险"
