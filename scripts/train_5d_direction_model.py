...
def direction_label(series: pd.Series) -> np.ndarray:
    ...
    return np.where(numeric.notna(), np.where(numeric > 0, "上涨", "下跌"), "")
...
def main() -> None:
    ...
    known_result_count = int(result["涨跌预测是否准确"].notna().sum())
    correct_count = int(result["涨跌预测是否准确"].sum(skipna=True))
    total_count = int(len(result))
    target_count = int(np.ceil(known_result_count * args.target_accuracy))
    accuracy = correct_count / known_result_count if known_result_count else np.nan
        ...
        [
            {
                ...
                "股票数量": total_count,
                "已知实际结果数": known_result_count,
                "预测正确数": correct_count,
                f"{target_pct}目标所需准确数": target_count,
                "涨跌预测准确率": f"{accuracy:.2%}" if pd.notna(accuracy) else "",
                f"是否达到{target_pct}目标": int(correct_count >= target_count) if known_result_count else "",
