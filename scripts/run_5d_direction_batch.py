...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--run-name", default="", help="批量运行名称；填写后输出到 output-dir/runs/<run-name>")
...
def effective_output_dir(args: argparse.Namespace) -> Path:
    ...
    if args.run_name:
        output_dir = output_dir / "runs" / args.run_name
...
def run_post_steps(args: argparse.Namespace, dates: list[str]) -> None:
    ...
    correction_cmd = [
        sys.executable,
        "scripts/postprocess/apply_market_risk_correction.py",
        "--output-dir",
        str(output_dir),
        "--dates",
        dates_arg,
    ...
    signal_cmd = [
        sys.executable,
        "scripts/postprocess/apply_signal_decision_layer.py",
        "--detail-csv",
        str(detail_csv),
        "--output-prefix",
        str(output_prefix),
...
def combine_run_outputs(args: argparse.Namespace, dates: list[str]) -> None:
    output_dir = effective_output_dir(args)
    ...
    detail_files = [
        path
        for path in sorted(output_dir.glob(f"03_每只股票方向预测结果*{args.output_suffix}_*.csv"))
        if not path.name.startswith("03_批量")
    ]
    stats_files = [
        path
        for path in sorted(output_dir.glob(f"04_每天方向预测准确率统计*{args.output_suffix}_*.csv"))
        if not path.name.startswith("04_批量")
    ]
    ...
    if detail_files:
        ...
        detail_path = output_dir / f"03_批量个股方向预测明细{args.output_suffix}_{dates[0]}_{dates[-1]}.csv"
        detail.to_csv(detail_path, index=False, encoding="utf-8-sig")
        print(f"已合并个股预测明细: {detail_path}")
    ...
    if stats_files:
        ...
        if "涨跌预测准确率" in stats.columns:
            stats["准确率数值"] = stats["涨跌预测准确率"].astype(str).str.rstrip("%").replace("", pd.NA).astype(float) / 100
        stats_path = output_dir / f"04_批量方向预测准确率汇总{args.output_suffix}_{dates[0]}_{dates[-1]}.csv"
        stats.to_csv(stats_path, index=False, encoding="utf-8-sig")
        print(f"已合并准确率汇总: {stats_path}")
...
def main() -> None:
    ...
    if failed:
        ...
        raise SystemExit(1)
    combine_run_outputs(args, dates)
