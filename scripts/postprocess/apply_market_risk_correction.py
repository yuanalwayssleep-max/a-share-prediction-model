...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data-dir", default="", help="00基础数据所在目录；默认等于output-dir")
    ...
    parser.add_argument("--market-prediction-csv", default="", help="06_市场5日方向预测结果CSV；提供后用于方向修正叠加层")
    parser.add_argument(
        "--correction-policy",
...
def market_model_correction_reason(row: pd.Series) -> str:
    """用市场方向模型叠加修正个股方向。

    市场模型只做方向修正，不承担最终交易出手判断。
    """
    market_direction = str(row.get("预测市场方向", ""))
    market_state = str(row.get("预测市场状态", ""))
    current = str(row.get("规则修正后预测涨跌", row.get("修正后预测涨跌", row.get("预测涨跌", ""))))
    raw = str(row.get("原始预测涨跌", row.get("预测涨跌", "")))
    up_prob = float(row.get("预测上涨概率", np.nan))
    blended = parse_percent(row.get("融合预测上涨比例", np.nan))
    risk_prob = parse_percent(row.get("风险模型强下跌概率", np.nan))
    opportunity_prob = parse_percent(row.get("机会模型强上涨概率", np.nan))

    if market_direction == "下跌" and current == "上涨":
        if market_state == "强下跌" or blended < 0.40 or risk_prob >= 0.50:
            return "市场模型强下跌，规则修正后上涨改下跌"
        if blended < 0.45 and up_prob < 0.80:
            return "市场模型偏下跌且个股上涨置信不足，规则修正后上涨改下跌"

    if market_direction == "上涨" and current == "下跌":
        if raw == "上涨" and up_prob >= 0.70:
            return "市场模型上涨且个股原始高置信上涨，规则修正后下跌改上涨"
        if blended >= 0.58 and opportunity_prob >= 0.40 and up_prob >= 0.60:
            return "市场模型上涨机会较强，规则修正后下跌改上涨"

    return "市场模型不修正"


def apply_market_model_direction(row: pd.Series) -> str:
    reason = str(row.get("市场模型修正原因", ""))
    if "改上涨" in reason:
        return "上涨"
    if "改下跌" in reason:
        return "下跌"
    return str(row.get("规则修正后预测涨跌", row.get("修正后预测涨跌", row.get("预测涨跌", ""))))
...
def main() -> None:
    ...
    data_dir = Path(args.data_dir).resolve() if args.data_dir else output_dir
    dates = [item.strip() for item in args.dates.split(",") if item.strip()]

    market = pd.read_csv(data_dir / "00_市场5日方向预测样本.csv", encoding="utf-8-sig")
    market_cols = [
        "日期",
        "未来5日上涨比例",
        "未来5日市场方向",
    ...
    index_feature_path = data_dir / "00_核心指数特征.csv"
    ...
    for date_tag in dates:
        ...
        if not path.exists():
            path = output_dir / "daily_detail" / f"03_每只股票方向预测结果_核心特征{args.prediction_suffix}_{date_tag}.csv"
        if not path.exists():
    ...
    if args.market_prediction_csv:
        market_prediction_path = Path(args.market_prediction_csv).resolve()
        market_prediction = pd.read_csv(market_prediction_path, encoding="utf-8-sig")
        market_prediction_cols = [
            col
            for col in [
                "锚点日期",
                "预测市场方向",
            ...
            if col in market_prediction.columns
        ]
        if "锚点日期" not in market_prediction_cols:
            raise ValueError(f"市场预测文件缺少锚点日期: {market_prediction_path}")
        market_prediction = market_prediction[market_prediction_cols].drop_duplicates("锚点日期")
        merged = merged.drop(
            columns=[col for col in market_prediction_cols if col != "锚点日期" and col in merged.columns],
            errors="ignore",
        ).merge(market_prediction, on="锚点日期", how="left")
    ...
    merged["规则修正后预测涨跌"] = merged.apply(corrected_direction, axis=1)
    merged["规则修正后预测是否准确"] = (merged["规则修正后预测涨跌"] == merged["实际涨跌"]).astype(int)
    if args.market_prediction_csv:
        merged["市场模型修正原因"] = merged.apply(market_model_correction_reason, axis=1)
    else:
        merged["市场模型修正原因"] = "未接入市场预测模型"
    merged["修正后预测涨跌"] = merged.apply(apply_market_model_direction, axis=1)
